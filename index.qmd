---
title: "Homework 2"
author: "[Conner L]{style='background-color: yellow;'}"
toc: true
...
```{R}
plot2 <- ggplot(df, aes(x = sex, y = length)) + geom_boxplot()
plot2
```

```{R}
plot3 <- ggplot(df, aes(x = sex, y = diameter)) + geom_violin()
plot3
```
...
```{R}
plot4 <- ggplot(df, aes(x = length, y = diameter, color = sex, size = shell_weight)) + geom_point()
plot4
```
...
```{R}
plot5 <- ggplot(df, aes(x = length, y = diameter)) + geom_point(aes(color = sex)) + geom_smooth(method = "lm", se = FALSE) + facet_wrap(~sex,, ncol = 1)
plot5
...
df %>% 
  ...
  ggtitle("tile plot for number of rings and sex") +
  xlab("Ring Num") +
  ylab("Sex")
...
```{R}
df_quant <- df %>% select(!sex)
df2 <- df %>% select(rings) 

plot5 <- map2(df_quant, df2, ~ ggplot(df) +
              geom_point(aes(x = rings, y = .x, color = sex)) + 
               labs(x = "Rings", y = " "))
cowplot::plot_grid(plotlist = plot5, labels = colnames(df_quant), ncol = 3, label_size = 10) 
...
for(i in new_diameters){
  new_heights <- append(new_heights, heighteq(i))
